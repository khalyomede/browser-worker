...
class BrowserWorker {
	...
	static async removeServiceWorker() {
		if ("serviceWorker" in navigator) {
			...
			Console.displayInfo(`${succeededCount} service workers removed, ${failedCount} service worker removals failed`);
		} else {
			Console.displayWarning("service workers are not supported in your browser");
		}
	...
	static async removeCaches() {
		if ("caches" in window) {
			...
			Console.displayInfo(`${succeedeedCount} caches removed, ${failedCount} cache removals failed`);
		} else {
