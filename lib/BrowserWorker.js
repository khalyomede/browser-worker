...
class BrowserWorker {
  ...
  static registerServiceWorker() {
    if ("serviceWorker" in navigator) {
      navigator.serviceWorker.register(BrowserWorker._serviceWorkerPath).then(serviceWorker => {
        // registration worked
  ...
  static listenRequests() {
    self.addEventListener("activate", event => {
      ...
      event.waitUntil(caches.keys().then(function (cacheNames) {
        return Promise.all(cacheNames.filter(cacheName => !BrowserWorker._activeCachesName.includes(cacheName)).map(cacheName => caches.delete(cacheName)));
      }));
    });
